...
let data_length = u32::from_le_bytes(
    bytes_raw[length_start..length_end].try_into().expect("bad data length")
) as usize;

    let extension = std::str::from_utf8(&bytes_raw[1..ext_end]).expect("utf8 for extension invalid").trim_matches(char::from(0)).trim();
    let mut output_filename_final = PathBuf::from(output_filename);
    output_filename_final.set_extension(extension);
    
    let stored_checksum = u32::from_le_bytes(bytes_raw[checksum_start..checksum_end].try_into().expect("couldn't decode checksum"));
